import fs from 'node:fs';
import path from 'node:path';
import {spawnSync} from 'node:child_process';
import {fileURLToPath} from 'node:url';
import test from 'ava';
import {temporaryDirectory} from 'tempy';
import gifsicle from '../index.js';

test('binary exists and reports a version', t => {
	const result = spawnSync(gifsicle, ['--version'], {encoding: 'utf8'});
	t.is(result.status, 0);
	t.regex(result.stdout, /Gifsicle/i);
});

test('minifies a gif', t => {
	const tmp = temporaryDirectory();
	const src = fileURLToPath(new URL('fixtures/test.gif', import.meta.url));
	const dst = path.join(tmp, 'test.gif');

	const result = spawnSync(gifsicle, ['-o', dst, src]);
	t.is(result.status, 0);

	const srcSize = fs.statSync(src).size;
	const dstSize = fs.statSync(dst).size;
	t.true(dstSize < srcSize, `expected ${dstSize} < ${srcSize}`);
});
